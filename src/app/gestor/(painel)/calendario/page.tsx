...
export default function CalendarioPage() {
  ...
  const [modalExcluir, setModalExcluir] = useState<Agendamento | null>(null);

  useEffect(() => {
    const raw = sessionStorage.getItem('gestorLogado');
    if (!raw) return;
    ...
    const novosDoEst = (novos as Agendamento[]).filter((a) => a.estabelecimentoId === g.estabelecimentoId);
  ...
  const salvarEdicao = () => {
    ...
    const novoProfissional = profissionaisList.find((p) => p.id === editProfId) || modalEditar.profissional;
    setAgendamentosList((prev) =>
      prev.map((a) =>
        a.id === modalEditar.id
          ? { ...a, data: editData, hora: editHora, status: editStatus, profissional: novoProfissional, observacoes: editObs }
          : a
      )
    );
  ...
  const diaSelecionadoFormatado = new Date(diaSelecionado + 'T12:00:00').toLocaleDateString('pt-BR', {
    weekday: 'long',
    day: 'numeric',
    month: 'long',
    year: 'numeric',
      ...
      <div className={styles.content}>
        <section className={styles.calendarioCard}>
          ...
          <div className={styles.legenda}>
            {(Object.entries(STATUS_COR) as [StatusAgendamento, string][]).map(([s, cor]) => (
                  ...
                  <div className={styles.agBody}>
                    <div className={styles.agTop}>
                      ...
                      <div className={styles.agRight}>
                        ...
                        <span className={styles.agValor}>R$ {ag.servico.preco.toFixed(2)}</span>
                      </div>
                    </div>
        ...
        {modalEditar && (
          <div className={styles.editForm}>
            <div className={styles.editHeader}>
              <div className={styles.agAvatar}>
                {modalEditar.clienteNome.split(' ').slice(0, 2).map((n) => n[0]).join('')}
              </div>
              <div>
            ...
            <div className={styles.editAcoes}>
              <button className={styles.btnSecundario} onClick={() => setModalEditar(null)}>
                Cancelar
              </button>
              <button className={styles.btnPrimario} onClick={salvarEdicao}>
                Salvar alterações
              </button>
            </div>
          </div>
      ...
      <Modal aberto={!!modalExcluir} onFechar={() => setModalExcluir(null)} titulo="Excluir Agendamento" largura="sm">
        {modalExcluir && (
          <div className={styles.excluirForm}>
            ...
            <div className={styles.editAcoes}>
              <button className={styles.btnSecundario} onClick={() => setModalExcluir(null)}>
                Cancelar
              </button>
              <button className={styles.btnPerigo} onClick={confirmarExclusao}>
                Excluir
              </button>
            </div>
          </div>
